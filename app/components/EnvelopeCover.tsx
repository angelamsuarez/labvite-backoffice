...
export default function EnvelopeCover({ onOpen }: EnvelopeCoverProps) {
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] flex flex-col items-center justify-center select-none transition-all duration-[900ms] ease-in-out ${
    ...
    >
      ...
      <div className="mb-2" onClick={(e) => isEditMode && e.stopPropagation()}>
        <EditableText
          ...
          as="p"
          className="text-[13px] tracking-[0.35em] uppercase font-playfair text-center"
          style={{ opacity: 0.8 }}
        />
      </div>

      {/* Names */}
      <div className="mb-20 text-center" onClick={(e) => isEditMode && e.stopPropagation()}>
        <EditableText
        ...
        />
        <span className="text-4xl md:text-5xl font-script mx-3" style={{ opacity: 0.9 }}>
          &amp;
        </span>
        <EditableText
      ...
      >
        <Image
          src="/envelope-rosa.svg"
          alt=""
          width={0}
          height={0}
          sizes="100vw"
          className="drop-shadow-lg pointer-events-none w-[300px] md:w-[400px] h-auto"
          priority
        />
      </div>
      ...
      <div className="mt-2" onClick={(e) => isEditMode && e.stopPropagation()}>
        {!isEditMode ? (
          <EditableText
            ...
            as="p"
            className="text-[11px] tracking-[0.3em] uppercase font-playfair text-center"
            style={{ opacity: 0.6 }}
          />
        ) : (
          <p
            className="text-[11px] font-playfair text-center tracking-wider"
            style={{ opacity: 0.4 }}
          >
